...
\begin{proof}
  ...
  spamming the neural interface. As the number of random seeds $N \to \infty$, the probability of finding
  $s^*$ approaches 1.
  \begin{equation}
...
\begin{proof}
  ...
  \begin{equation}
    \theta_{t+1} = \mathcal{Q}\lrb{\texttt{torch.manual\_seed}(\text{randint}(0, 2^{128}))}
  \end{equation}
...
\begin{proof}
  ...
  $O(N^3)$ complexity of Transformers.
\end{proof}
